...
import { User, Robot, Brain, CaretRight, Sparkle } from '@phosphor-icons/react';
        ...
        >
            <div className="relative overflow-hidden rounded-2xl border border-cyan-500/20 bg-black/60 p-8 text-white shadow-[0_0_40px_-10px_rgba(6,182,212,0.15)] backdrop-blur-2xl">
                ...
                <div className="mb-8 text-center">
                    <div className="mx-auto mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-cyan-500/10 border border-cyan-500/30">
                        <Sparkle size={20} weight="fill" className="text-cyan-400" />
                    </div>
                    <h1 className="mb-2 text-xl font-bold tracking-widest uppercase text-cyan-400">System Configuration</h1>
                    <div className="h-0.5 w-16 mx-auto bg-cyan-500/50 rounded-full"></div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            <label className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-cyan-200/50">
                            ...
                            <input
                                ...
                                className="w-full rounded-md border border-cyan-500/20 bg-cyan-950/20 px-3 py-2 text-sm text-cyan-100 placeholder-cyan-900/50 transition-all focus:border-cyan-400 focus:bg-cyan-950/40 focus:outline-none focus:ring-1 focus:ring-cyan-400"
                            />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold uppercase tracking-wider text-cyan-200/50 pl-1">
                                Last Name
                            </label>
                            <input
                                type="text"
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            <select
                                ...
                                onChange={(e) => setFormData({ ...formData, assistantBehavior: e.target.value as any })}
                                className="w-full appearance-none rounded-md border border-cyan-500/20 bg-cyan-950/20 px-3 py-2 text-sm text-cyan-100 transition-all focus:border-cyan-400 focus:outline-none focus:ring-1 focus:ring-cyan-400"
                            >
                                {BEHAVIORS.map((b) => (
                    ...
                    {/* Error Message */}
                    {error && (
                        <p className="text-center text-xs text-red-400 font-mono animate-pulse">ERROR: {error}</p>
                    )}

                    {/* Submit Button */}
                    <button
                        type="submit"
                        className="group w-full rounded-md bg-cyan-500 hover:bg-cyan-400 text-black py-3 text-xs font-bold uppercase tracking-widest shadow-[0_0_20px_-5px_rgba(34,211,238,0.6)] transition-all active:scale-[0.98]"
                    >
